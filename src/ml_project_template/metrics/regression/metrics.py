...
def set_regression_metrics(
    ...
    """
    nrmse = NormalizedRootMeanSquaredError(normalization=nrmse_normalization_technique, num_outputs=num_outputs, device=device)
...
def regression_report(
    ...
    nrmse, mse = set_regression_metrics(num_outputs=num_outputs, device=device, nrmse_normalization_technique=nrmse_normalization_technique, mse_squared=mse_squared)
    
    return {
        "Normalized Root Mean Squared Error": nrmse(y_pred, y_true).item(),
        "Mean Squared Error" if mse_squared else "Root Mean Squared Error": mse(y_pred, y_true).item()
    }