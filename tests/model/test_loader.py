...
def test_invalid_task_type_raises_value_error():
    with pytest.raises(ValueError, match="Invalid task type, supported values are: 'binary', 'multiclass', 'regression'"):
...
def test_invalid_model_path_raises_value_error():
    ...
    with pytest.raises(ValueError, match="The model path is invalid, please verify if the model has the correct extention, or that the path exists."):
        model.preload()
